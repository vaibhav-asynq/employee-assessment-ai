...
export function Path2Provider({ children }: { children: React.ReactNode }) {
  const [editableData, setEditableData] = useState<InterviewAnalysis | null>(
    null,
  );
  const [initialized, setInitialized] = useState(false);

  const setEditableDataWithInit = useCallback(
    (data: InterviewAnalysis | null) => {
  ...
  const updateData = useCallback(
    (data: Partial<InterviewAnalysis>) => {
