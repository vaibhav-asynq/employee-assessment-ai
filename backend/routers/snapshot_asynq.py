...
async def create_snapshot_endpoint(
    ...
    task = await async_get_task_by_user_and_fileId(user_id, request.file_id, db)
    ...
    if request.trigger_type == "auto":
        last_auto_snapshot = await async_get_last_auto_snapshot(db, task.id)
        if last_auto_snapshot:
            await async_delete_snapshot(db, last_auto_snapshot.id)
    
    # Create snapshot
    snapshot = await async_create_snapshot(
        db, 
        snapshot_data, 
    ...
    if make_current:
        await async_restore_snapshot(db, task.id, snapshot.id)
...
async def get_latest_snapshot_endpoint(
    ...
    # Get latest snapshot
    snapshot = await async_get_latest_snapshot(db, task.id)
...
async def get_current_snapshot_endpoint(
    ...
    snapshot = await async_get_current_snapshot(db, task.id)
...
async def get_snapshot_history_endpoint(
    ...
    snapshots = await async_get_snapshot_history(db, task.id, limit, offset)
...
async def get_manual_snapshots_endpoint(
    ...
    snapshots = await async_get_manual_snapshots(db, task.id, limit, offset)
...
async def get_snapshots_by_type_endpoint(
    ...
    snapshots = await async_get_snapshots_by_type(db, task.id, trigger_type, limit, offset)
...
async def count_snapshots_endpoint(
    ...
    # Count snapshots
    count = await async_count_snapshots(db, task.id, trigger_type)
...
async def restore_snapshot_endpoint(
    ...
    # Restore snapshot
    snapshot = await async_restore_snapshot(db, task.id, snapshot_id)
...
async def undo_snapshot_endpoint(
    ...
    # Undo snapshot
    snapshot = await async_undo_snapshot(db, task.id)
...
async def redo_snapshot_endpoint(
    ...
    # Redo snapshot
    snapshot = await async_redo_snapshot(db, task.id)
...
async def delete_snapshot_endpoint(
    ...
    snapshot = await async_get_snapshot_by_id(db, snapshot_id)
    if not snapshot:
        apiLogger.warning(f"Snapshot not found: {snapshot_id}")
        raise HTTPException(status_code=404, detail="Snapshot not found")
    
    # Get task to verify ownership
    task = await async_get_db_task(snapshot.task_id, user_id, db)
    if not task:
    ...
    # Delete snapshot
    success = await async_delete_snapshot(db, snapshot_id)
...
async def get_snapshot_children_endpoint(
    ...
    snapshot_result = await async_get_snapshot_with_children(db, snapshot_id)
    
    if not snapshot_result or not snapshot_result[0]:
        raise HTTPException(status_code=404, detail="Snapshot not found")
    
    snapshot, children = snapshot_result
